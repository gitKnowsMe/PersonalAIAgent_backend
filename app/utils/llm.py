...
# PHASE 1 FIX: Enhanced response cache with query-specific hashing and metadata
_response_cache = {}
_cache_metadata = {}  # Stores cache creation time and query context info
...
def clear_response_cache():
    ...
    """
    global _response_cache, _cache_metadata
    logger.info("Clearing response cache")
    _response_cache.clear()
    _cache_metadata.clear()

def generate_enhanced_cache_key(query: str, context_chunks: List[Any], model_name: str = None) -> str:
    """
    PHASE 1 FIX: Generate enhanced cache key that includes query intent and context signature
    """
    import time
    ...
    # Extract key context signatures to differentiate responses
    context_signatures = []
    for chunk in context_chunks[:3]:  # Use first 3 chunks for signature
        if isinstance(chunk, dict):
            content = chunk.get('content', '') or chunk.get('text', '')
        else:
            content = str(chunk)
        
        # Create a signature based on first 100 chars of content
        signature = hashlib.md5(content[:100].encode()).hexdigest()[:8]
        context_signatures.append(signature)
    
    # Include model name and query intent for differentiation
    query_intent = classify_query_type(normalized_query)[0].value if context_chunks else 'general'
...
def is_cache_valid(cache_key: str, max_age_seconds: int = 3600) -> bool:
    """Check if cached response is still valid (not too old or stale)"""
    if cache_key not in _cache_metadata:
        return False
    
    metadata = _cache_metadata[cache_key]
...
def get_current_model_info():
    ...
    # Fallback: detect from settings
    current_path = settings.LLM_MODEL_PATH
    model_name = None
    for name, filename in AVAILABLE_MODELS.items():
...
def classify_query_type(query: str) -> Tuple[QueryType, float]:
    ...
    general_ai_keywords = [
        ...
        "neural", "network", "algorithm", "ai", "deep learning", "technology",
        "prompt", "prompting", "sampling", "top-p", "top-k", "temperature", 
        "chain of thought", "cot", "llm", "model", "training", "transformer"
    ]
    personal_data_keywords = [
        "my", "i", "me", "personal", "document", "file", "upload", "search"
    ]
    
    # Count keyword matches (using word boundaries to avoid substring matches)
    import re
    query_words = query_lower.split()
    
    # Handle multi-word phrases first
    if "tell me" in query_lower:
        # "Tell me" is a general knowledge pattern, override personal "me"
        general_matches = 2  # Give strong weight to "tell me"
        personal_matches = 0  # Don't count "me" when it's part of "tell me"
    else:
        general_matches = sum(1 for keyword in general_ai_keywords if keyword in query_words or any(keyword in word for word in query_words if keyword in ['tell me']))
        personal_matches = sum(1 for keyword in personal_data_keywords if keyword in query_words)
...
def generate_prompt(query: str, context_chunks: List[str], first_person_mode: bool = False, model_name: str = None) -> str:
    ...
    # Adjust max context based on model - Phi-2 needs much smaller context due to 2048 token limit
    if model_name == "phi-2":
        max_context_chars = 1000  # Conservative limit for Phi-2's small context window
        max_chunk_chars = 300     # Smaller chunks for Phi-2
    else:
        max_context_chars = 6000  # Larger models can handle more context
    ...
    for i, chunk in enumerate(context_chunks):
        if len(formatted_context) >= max_context_chars:
            ...
            break
        # Truncate individual chunk based on model capacity
        safe_chunk = chunk[:max_chunk_chars]
        formatted_context += f"{safe_chunk}\n\n"
    
    # Create model-specific prompts
    if model_name == "phi-2":
        # Phi-2 requires ChatML format to work properly
        if not formatted_context.strip():
            # General knowledge query - use ChatML format
            prompt = f"""<|im_start|>user
{query}<|im_end|>
<|im_start|>assistant
"""
        else:
            # Document-based query - use ChatML format with context
            prompt = f"""<|im_start|>user
Context:
{formatted_context}
...
Please provide a clear, concise answer based on the context above.<|im_end|>
<|im_start|>assistant
"""
    else:
...
def generate_response(query: str, context_chunks: List[Any], first_person_mode: bool = False, model_name: str = None) -> str:
    ...
    try:
        ...
        # PHASE 1 FIX: Allow more context chunks for better responses - take only the most relevant chunks
        limited_context = context_content[:6]  # PHASE 1: Increased from 3 to 6 most relevant chunks maximum
        ...
        if model_name == "phi-2":
            cleaned_response = cleaned_response.replace("<|im_start|>", "").replace("<|im_end|>", "").strip()
            cleaned_response = cleaned_response.replace("assistant", "").strip()
        else:
        ...
        if not cleaned_response or len(cleaned_response) < ai_config.min_response_length:
            ...
            # Check if this is a general knowledge query (no context provided)
            if not context_content:
                return "I'm having trouble generating a response to that question. Could you try rephrasing it or asking in a different way?"
            else:
...
async def generate_answer(query: str, context_chunks: List[Dict[Any, Any]]) -> tuple[str, bool]:
    ...
    try:
        ...
        if getattr(ai_config, "enable_response_caching", False):
            # PHASE 1 FIX: Use enhanced cache key generation with query intent and context signatures
            cache_key = generate_enhanced_cache_key(query, context_chunks)
            
            # Check if we have a valid cached response
            if cache_key in _response_cache and is_cache_valid(cache_key):
                logger.info(f"Returning valid cached response for query: '{query}' (cache_key: {cache_key[:8]}...)")
                return _response_cache[cache_key], True
            elif cache_key in _response_cache:
                # Cache exists but is stale, remove it
                logger.info(f"Removing stale cache entry for query: '{query}'")
                del _response_cache[cache_key]
                if cache_key in _cache_metadata:
                    del _cache_metadata[cache_key]
        ...
        # PHASE 1 FIX: Cache the response with enhanced metadata if caching is enabled
        if getattr(ai_config, "enable_response_caching", False) and cache_key is not None:
            _response_cache[cache_key] = response
            logger.info(f"Cached enhanced response for query: '{query}' (cache_key: {cache_key[:8]}...)")
