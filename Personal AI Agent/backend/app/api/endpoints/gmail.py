...
async def initiate_gmail_auth(
    ...
    try:
        ...
        # Create new session with longer expiration
...
async def gmail_oauth_callback(
    ...
    try:
        ...
        if not oauth_session:
            ...
            from fastapi.responses import RedirectResponse
            return RedirectResponse(url="/?error=oauth_expired", status_code=302)
        ...
        from fastapi.responses import RedirectResponse
        return RedirectResponse(url=f"/?gmail_connected={email_account.email_address}", status_code=302)
        
    except Exception as e:
        logger.error(f"Error in OAuth callback: {e}")
        from fastapi.responses import RedirectResponse
        return RedirectResponse(url=f"/?error=oauth_failed&message={str(e)}", status_code=302)
