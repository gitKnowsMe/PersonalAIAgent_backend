from sqlalchemy import Column, ForeignKey, Integer, String, Text, DateTime, Boolean, CheckConstraint, Index
...
class EmailAccount(Base):
    ...
    __table_args__ = (
        CheckConstraint("email_address LIKE '%@%'", name='email_account_format'),
        CheckConstraint("provider IN ('gmail', 'outlook')", name='valid_email_provider'),
        # Performance indexes
...
class Email(Base):
    ...
    recipient_emails = Column(Text, nullable=True)  # JSON array of recipient emails
    cc_emails = Column(Text, nullable=True)  # JSON array of CC emails
    bcc_emails = Column(Text, nullable=True)  # JSON array of BCC emails
    body_text = Column(Text, nullable=True)  # Plain text body
    body_html = Column(Text, nullable=True)  # HTML body
    email_type = Column(String(20), nullable=False, default="generic")  # 'business', 'personal', 'promotional', 'transactional', 'support'
    is_read = Column(Boolean, default=False, nullable=False)
    is_important = Column(Boolean, default=False, nullable=False)
    has_attachments = Column(Boolean, default=False, nullable=False)
    gmail_labels = Column(Text, nullable=True)  # JSON array of Gmail labels
    sent_at = Column(DateTime(timezone=True), nullable=False, index=True)
    created_at = Column(DateTime(timezone=True), server_default=func.now(), nullable=False)
    vector_namespace = Column(String(200), nullable=True)  # For vector storage
    ...
    __table_args__ = (
        CheckConstraint("sender_email LIKE '%@%'", name='sender_email_format'),
        CheckConstraint("email_type IN ('business', 'personal', 'promotional', 'transactional', 'support', 'generic')", name='valid_email_type'),
        CheckConstraint('LENGTH(subject) <= 500', name='subject_max_length'),
        # Performance indexes - High Priority (Foreign Keys)
        Index('idx_emails_user_id', 'user_id'),
        Index('idx_emails_email_account_id', 'email_account_id'),
        # Performance indexes - Medium Priority (Filtering)
        ...
        Index('idx_emails_account_sent_at_composite', 'email_account_id', 'sent_at'),
...
class EmailAttachment(Base):
    ...
    file_path = Column(String(500), nullable=True)  # Local file path if downloaded
    file_size = Column(Integer, nullable=False)
    mime_type = Column(String(100), nullable=False)
    attachment_id = Column(String(255), nullable=False)  # Gmail attachment ID
    ...
    __table_args__ = (
        CheckConstraint('file_size > 0', name='attachment_file_size_positive'),
        CheckConstraint('LENGTH(filename) >= 1', name='attachment_filename_not_empty'),
        # Performance indexes
...
class OAuthSession(Base):
    ...
    __table_args__ = (
        CheckConstraint("provider IN ('gmail', 'outlook')", name='valid_oauth_provider'),
        CheckConstraint('LENGTH(session_id) >= 10', name='session_id_min_length'),
        # Performance indexes
