...
class GmailService:
    ...
    def __init__(self):
        ...
        self._refreshing_accounts: Dict[str, bool] = {}
    ...
    def _get_refresh_lock(self, account_id: str) -> asyncio.Lock:
        """Get or create a refresh lock for the given account."""
    ...
    async def refresh_access_token(self, email_account: EmailAccount, db: Session) -> bool:
        """
        Thread-safe refresh access token using refresh token with account-specific locking
        ...
        refresh_lock = self._get_refresh_lock(account_id)
        
        try:
            # Acquire lock with timeout to prevent deadlocks
            await asyncio.wait_for(refresh_lock.acquire(), timeout=self._lock_timeout)
            
            try:
                # Double-check if token still needs refresh after acquiring lock
                db.refresh(email_account)  # Refresh database state
                if not self._is_token_near_expiry(email_account.token_expires_at):
                    logger.info(f"Token for {account_id} was already refreshed by another request")
                    return True
                
                # Check if another request is already refreshing this account
                if self._refreshing_accounts.get(account_id, False):
                    logger.info(f"Token refresh already in progress for {account_id}")
                    return True
                ...
                # Mark account as being refreshed
                self._refreshing_accounts[account_id] = True
                ...
                try:
                    ...
                    # Always clear the refreshing status
                    self._refreshing_accounts.pop(account_id, None)
            ...
            logger.error(f"Timeout waiting for refresh lock for account {account_id}")
            raise TokenRefreshError(f"Timeout waiting for token refresh lock for account {account_id}")
        except Exception as e:
            logger.error(f"Error refreshing access token for {account_id}: {e}")
            self._refreshing_accounts.pop(account_id, None)  # Clear status on error
    ...
    async def disconnect_account(self, email_account: EmailAccount, db: Session) -> bool:
        ...
        try:
            ...
            # Clear tokens and deactivate account
            ...
            logger.info(f"Disconnected Gmail account: {email_account.email_address}")
            return True
            
        except Exception as e:
            logger.error(f"Error disconnecting account {email_account.email_address}: {e}")
            
            # Still return False for disconnect failures, but log appropriately
            if "network" in str(e).lower() or "connection" in str(e).lower():
                logger.error(f"Network error during account disconnect: {e}")
            else:
                logger.error(f"Unexpected error during account disconnect: {e}")
